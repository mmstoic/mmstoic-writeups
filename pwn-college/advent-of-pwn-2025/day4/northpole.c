...
int main(void)
{
    ...
    //Find the eBPF program handle_do_linkat
    ...
    //Attaches the eBPF program to a kprobe, essentially making our eBPF program
    // fire when the linkat() syscall is called. linkat() is essentially a modern version of 
    // link(), which creates a hard link between files. The "false" flag means that the eBPF
    // program will fire at linkat()'s entry, not exit.
    ...
    //Handle SIGINT and SIGTERM with these signal handlers
    signal(SIGINT, handle_sigint);
    signal(SIGTERM, handle_sigint);

    //Find the map that kernel and user-space use to communicate 
    ...
    //When this program is run with the init shell program its stdout and stderr are discarded
    printf("Attached. Press Ctrl-C to quit.\n");
    fflush(stdout);

    /*
    The eBPF program writes some value into the 0th index of the map data structure (we which we can assume is some kind of array).
    If the value that is written here is non-zero, change should_broadcast to 1. Then, the broadcast_cheer() function 
    will be called.
    */
    while (!stop) {
        __u32 v = 0;
